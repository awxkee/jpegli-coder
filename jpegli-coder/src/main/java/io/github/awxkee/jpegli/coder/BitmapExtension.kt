 ...
 * @see Scalar
 */
fun Bitmap.compressJpegli(
    outputStream: OutputStream, @IntRange(from = 0, to = 100) quality: Int,
    strategy: IccStrategy = IccStrategy.XYB,
    background: Scalar = Scalar(255),
    progressive: Boolean = false,
) {
    JpegliCoder.compress(this, quality, strategy, background, progressive, outputStream)
}